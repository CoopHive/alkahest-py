import asyncio
from alkahest_py import PyTestEnvManager, PyMockERC1155, PyMockERC721
...
async def test_pay_erc1155_for_erc721():
    ...
    try:
        ...
        if alice_now_owns_erc721.lower() != env.alice.lower():
            raise Exception(f"Alice should have received the ERC721 token, but it's owned by {alice_now_owns_erc721}")
        
        if bob_received_erc1155 != 5:
            raise Exception(f"Bob should have received 5 ERC1155 tokens, got {bob_received_erc1155}")
        ...
        print("✅ test_pay_erc1155_for_erc721 PASSED")
        return True
        
    except Exception as e:
        print(f"❌ test_pay_erc1155_for_erc721 FAILED: {e}")
        raise


async def main():
    try:
        success = await test_pay_erc1155_for_erc721()
