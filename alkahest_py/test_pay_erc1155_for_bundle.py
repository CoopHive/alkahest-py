...
async def test_pay_erc1155_for_bundle():
    ...
    try:
        ...
        if bob_erc20_balance != 20:
            raise Exception(f"Bob should have 20 ERC20 tokens, got {bob_erc20_balance}")
        ...
        if bob_erc1155_balance != 4:
            raise Exception(f"Bob should have 4 ERC1155B tokens, got {bob_erc1155_balance}")
        ...
        if final_alice_erc20_balance - initial_alice_erc20_balance != 20:
            raise Exception(f"Alice should have received 20 ERC20 tokens, got {final_alice_erc20_balance - initial_alice_erc20_balance}")
        
        if alice_erc721_owner.lower() != env.alice.lower():
            raise Exception(f"Alice should have received the ERC721 token from bundle, but it's owned by {alice_erc721_owner}")
        
        if final_alice_erc1155b_balance - initial_alice_erc1155b_balance != 4:
            raise Exception(f"Alice should have received 4 ERC1155B tokens, got {final_alice_erc1155b_balance - initial_alice_erc1155b_balance}")
        
        # Verify Bob received the ERC1155
        if final_bob_erc1155a_balance - initial_bob_erc1155a_balance != 5:
            raise Exception(f"Bob should have received 5 ERC1155A tokens, got {final_bob_erc1155a_balance - initial_bob_erc1155a_balance}")
        ...
        print("✅ test_pay_erc1155_for_bundle PASSED")
        return True
        
    except Exception as e:
        print(f"❌ test_pay_erc1155_for_bundle FAILED: {e}")
        raise


async def main():
    try:
        success = await test_pay_erc1155_for_bundle()
