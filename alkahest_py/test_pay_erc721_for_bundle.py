...
async def test_pay_erc721_for_bundle():
    ...
    try:
        ...
        if bob_erc1155_balance != 5:
            raise Exception(f"Bob should have 5 ERC1155 tokens, got {bob_erc1155_balance}")
        ...
        if final_alice_erc1155_balance - initial_alice_erc1155_balance != 5:
            raise Exception(f"Alice should have received 5 ERC1155 tokens, got {final_alice_erc1155_balance - initial_alice_erc1155_balance}")
        
        # Verify Bob received the ERC721
        if bob_token_owner.lower() != env.bob.lower():
            raise Exception(f"Bob should have received the ERC721 token, but it's owned by {bob_token_owner}")
        
        print("✅ test_pay_erc721_for_bundle PASSED")
        return True
        
    except Exception as e:
        print(f"❌ test_pay_erc721_for_bundle FAILED: {e}")
        raise


if __name__ == "__main__":
    asyncio.run(test_pay_erc721_for_bundle())
