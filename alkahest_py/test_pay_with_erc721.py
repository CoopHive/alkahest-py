...
async def test_pay_with_erc721():
    ...
    try:
        ...
        if final_owner.lower() != env.bob.lower():
            raise Exception(f"Bob should own the ERC721 token, but it's owned by {final_owner}")
        
        print("✅ test_pay_with_erc721 PASSED")
        return True
        
    except Exception as e:
        print(f"❌ test_pay_with_erc721 FAILED: {e}")
        raise


if __name__ == "__main__":
    asyncio.run(test_pay_with_erc721())
