...
async def test_pay_erc1155_for_erc1155():
    ...
    try:
        ...
        if alice_received_b != 3:
            raise Exception(f"Alice should have received 3 tokens B, got {alice_received_b}")
        
        if bob_received_a != 5:
            raise Exception(f"Bob should have received 5 tokens A, got {bob_received_a}")
        ...
        print("✅ test_pay_erc1155_for_erc1155 PASSED")
        return True
        
    except Exception as e:
        print(f"❌ test_pay_erc1155_for_erc1155 FAILED: {e}")
        raise


async def main():
    try:
        success = await test_pay_erc1155_for_erc1155()
