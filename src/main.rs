use alkahest_rs::{
    clients::oracle::{
        ArbitrateOptions, AttestationFilter, AttestationFilterWithoutRefUid, Decision,
        EscrowParams, FulfillmentParams, FulfillmentParamsWithoutRefUid,
    },
...
use alloy::{
    ...
    primitives::{bytes, Address, Bytes, FixedBytes},
...
use {
    alkahest_rs::clients::arbiters::{
        ...
        SpecificAttestationArbiter, TrustedOracleArbiter, TrustedPartyArbiter, UidArbiterComposing,
    },
...
// Required main function for binary crate
fn main() {}
...
#[tokio::test]
async fn test_conditional_listen_and_arbitrate_for_escrow_no_spawn() -> eyre::Result<()> {
        ...
        oracle
            .listen_and_arbitrate_for_escrow_no_spawn(
                ...
                &|_statement: &StringObligation::StatementData,
                  _demand: &TrustedOracleArbiter::DemandData| {
                    println!(
    ...
    let bad_fulfillment_uid2 = make_fulfillment(&test, "bad2", escrow_uid).await?;
    tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
